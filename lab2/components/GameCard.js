...
export default function GameCard({ gameData }) {
  return (
    <Card>
      <GameImage source={gameData.image} />
      <Overlay>
        <GameTitle>{gameData.title}</GameTitle>
        <Subtitle>{gameData.subtitle}</Subtitle>
        <BottomRow>
          <PriceRow>
            <DiscountTag>{gameData.discount}</DiscountTag>
            <OldPrice>{gameData.price}</OldPrice>
            <NewPrice>{gameData.discountedPrice}</NewPrice>
          </PriceRow>
          {gameData.icons.map((icon, index) => (
